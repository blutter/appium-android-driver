...
async function webviewsFromProcs (adb, deviceSocket) {
  ...
  for (let line of out.split("\n")) {
    ...
    // let addIfwebView = (line, webviewSuffix, webviewArray) => {
    //   let webviewPid;
    //   let crosswalkWebviewSocket;
    //   if ((webviewPid = line.match(new RegExp(WEBVIEW_REGEXP)))) {
    //     webviewArray.push(WEBVIEW_BASE + webviewPid[1]);
    //   } else if ((crosswalkWebviewSocket = line.match(new RegExp("@" + CROSSWALK_REGEXP)))) {
    //     webviewArray.push(WEBVIEW_BASE + crosswalkWebviewSocket[1] + webviewSuffix);
    //   }
    // };
    ...
    if (deviceSocket) {
      if (line.indexOf(`@${deviceSocket}`) === line.length - deviceSocket.length - 1) {
        ...
        // } else {
        //   addIfwebView(line, "", webviews);
        // }
      }
    } 
    // else {
    //   addIfwebView(line, CROSSWALK_SOCKET_SUFFIX, webviews);
    // }
    ...
    if ((webviewPid = line.match(new RegExp(WEBVIEW_REGEXP)))) {
      ...
      console.log(crosswalkWebviewSocket);
      if (deviceSocket)
      {
...
helpers.getWebviews = async function (adb, deviceSocket) {
  ...
  console.log("get wv returning " + webviews);
  return webviews;
};
